...
{	
	...
	//------------------------------------------------------------------------------------------------
	//! Get BigBox class for faction (default: Current faction)
	S7R_BigBox GetBigBox7RForFaction(ELoadouts7R factionkey = ELoadouts7R.None)
	{
		S7R_FactionLoadouts factionLoadout;
		if (factionkey == 0)
			factionLoadout = m_7RLoadoutImports.GetFactionConfigFromFactionKey(m_eFactionLoadout7RKey);
		else
			factionLoadout = m_7RLoadoutImports.GetFactionConfigFromFactionKey(factionkey);
		
		if (!factionLoadout)
			return null;

		return factionLoadout.GetBigBox();
	}
	
	//------------------------------------------------------------------------------------------------
	//! Get SupplyBox faction (default: Current faction)
	S7R_StructSupplyLoadouts GetSupplyBox7RForFaction(ESupplyBoxType supplyType, int typeID = 0, ELoadouts7R factionkey = ELoadouts7R.None)
	{
		S7R_FactionLoadouts factionLoadout;
		if (factionkey == 0)
			factionLoadout = m_7RLoadoutImports.GetFactionConfigFromFactionKey(m_eFactionLoadout7RKey);
		...
		S7R_SupplyBoxLoadouts supplyBoxConfig = factionLoadout.GetSupplyBoxLoadouts();
	...
	//------------------------------------------------------------------------------------------------
	//! Get VehicleLoadout faction (default: Current faction)
	S7R_StructVehicleLoadout GetVehicleLoadout7RForFaction(EVehicleLoadoutType vehicleLoadoutType, int typeID = 0, ELoadouts7R factionkey = ELoadouts7R.None)
	{
		S7R_FactionLoadouts factionLoadout;
		if (factionkey == 0)
			factionLoadout = m_7RLoadoutImports.GetFactionConfigFromFactionKey(m_eFactionLoadout7RKey);
		...
		S7R_VehicleLoadouts vehicleLoadoutConfig = factionLoadout.GetVehicleLoadouts();
	...
	protected bool IsValid7RLoadout(SCR_PlayerArsenalLoadout loadout)
	{
		...
		{
			Print("[SCR_LoadoutManager: IsValidLoadout] No faction key found for selected SCR_Role7R", LogLevel.ERROR);
			return false;
		}
